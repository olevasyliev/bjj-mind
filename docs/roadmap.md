...
**Architecture:** Swift + SwiftUI frontend, Supabase backend (auth + DB + realtime), Claude API for AI features, Babylon.js (WKWebView) for 3D BJJ scenes.

**Tech Stack:** Swift 6.0, SwiftUI, Supabase (Swift SDK), Claude API, Babylon.js (WKWebView), Lottie (animations)
...
- ✅ Xcode project via xcodegen (see `BJJMind/`)
- Swift 6.0, SwiftUI, iOS 16+
- Localization: L10n enum + Localizable.strings for EN/ES/PT (strings never hardcoded)
- XCTest for TDD
